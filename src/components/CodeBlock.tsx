        ...
        <pre className="p-3 md:p-4 text-[11px] md:text-[13px] font-mono leading-6 overflow-x-auto">
          {lines.map((line, i) => {
            ...
            return (
              <div
                key={i}
                className={`rounded-sm font-semibold transition-colors duration-500 ${
                  highlight?.type === 'unsafe'
              ...
              >
                <span
                  className={`text-black/50 select-none mr-3 inline-block w-3 text-right text-[10px] ${highlight ? 'ml-1' : ''}`}
                >
                  {i + 1}
                </span>
